
const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');

function isUniversalEditorMode(block) {
  return !!block.querySelector('[data-aue-resource], [data-aue-type], [data-aue-prop], [data-aue-label]')
    || !!document.querySelector('[data-aue-resource], [data-aue-type], [data-aue-prop], [data-aue-label]');
}

function extractVimeoIdFromLink(href) {
  try {
    const url = new URL(href);
    const parts = url.pathname.split('/').filter(Boolean);
    for (let i = parts.length - 1; i >= 0; i -= 1) {
      if (/^\d+$/.test(parts[i])) return parts[i];
    }
    return '';
  } catch (e) {
    return '';
  }
}

function buildVimeoEmbed(videoId, autoplay) {
  const params = new URLSearchParams();
  params.set('autoplay', autoplay ? '1' : '0');
  params.set('controls', '1');
  if (autoplay) params.set('muted', '1');
  params.set('background', '0');

  const wrap = document.createElement('div');
  wrap.className = 'video-embed';
  wrap.style.left = '0';
  wrap.style.width = '100%';
  wrap.style.height = '0';
  wrap.style.position = 'relative';
  wrap.style.paddingBottom = '56.25%';

  const iframe = document.createElement('iframe');
  iframe.src = `https://player.vimeo.com/video/${encodeURIComponent(videoId)}?${params.toString()}`;
  iframe.style.border = '0';
  iframe.style.top = '0';
  iframe.style.left = '0';
  iframe.style.width = '100%';
  iframe.style.height = '100%';
  iframe.style.position = 'absolute';
  iframe.setAttribute('allow', 'autoplay; fullscreen; picture-in-picture');
  iframe.setAttribute('allowfullscreen', '');
  iframe.setAttribute('loading', 'lazy');
  iframe.setAttribute('title', 'Content from Vimeo');

  wrap.appendChild(iframe);
  return { wrap, iframe };
}

function parseItemsFromBlock(block) {
  const rows = Array.from(block.querySelectorAll(':scope > div'));

  return rows.map((row) => {
    const cells = Array.from(row.querySelectorAll(':scope > div'));
    const left = cells[0] || row;
    const right = cells[1] || row;

    const picture = left.querySelector('picture') || row.querySelector('picture');
    const linkEl = right.querySelector('a') || row.querySelector('a');
    const href = linkEl ? linkEl.href : '';
    const label = (linkEl && (linkEl.textContent || '').trim()) || '';

    const img = picture ? picture.querySelector('img') : null;
    const thumb = img ? {
      src: img.getAttribute('src') || '',
      alt: img.getAttribute('alt') || 'Thumbnail',
      title: img.getAttribute('title') || '',
      width: img.getAttribute('width') || '',
      height: img.getAttribute('height') || '',
    } : null;

    return { row, picture, href, label, thumb };
  }).filter((x) => x.href);
}

function decoratePublished(block, items) {
  const autoplayRequested = true;

  const videosWrap = document.createElement('div');
  videosWrap.className = 'video-items';

  const playlistWrap = document.createElement('div');
  playlistWrap.className = 'video-series__playlist';

  const loadVideoForItem = (itemEl) => {
    if (itemEl.dataset.embedLoaded === 'true') return;

    const href = itemEl.dataset.vimeoLink || '';
    const videoId = extractVimeoIdFromLink(href);
    if (!videoId) return;

    const autoplayActual = autoplayRequested && !prefersReducedMotion.matches;
    const { wrap, iframe } = buildVimeoEmbed(videoId, autoplayActual);
    itemEl.appendChild(wrap);

    iframe.addEventListener('load', () => {
      itemEl.dataset.embedLoaded = 'true';
      const ph = itemEl.querySelector('.video-placeholder');
      if (ph) ph.remove();
    });
  };

  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (!entry.isIntersecting) return;
      const itemEl = entry.target;
      observer.unobserve(itemEl);
      loadVideoForItem(itemEl);
    });
  }, { rootMargin: '200px 0px' });

  items.forEach(({ picture, href, label, thumb }, index) => {
    const itemIndex = index + 1;
    const itemId = `video-item-${itemIndex}`;

    const itemEl = document.createElement('div');
    itemEl.className = 'video-item';
    itemEl.id = itemId;
    itemEl.dataset.embedLoaded = 'false';
    itemEl.dataset.vimeoLink = href;

    if (picture) {
      const placeholder = document.createElement('div');
      placeholder.className = 'video-placeholder';
      placeholder.appendChild(picture);
      itemEl.appendChild(placeholder);
    }

    videosWrap.appendChild(itemEl);
    observer.observe(itemEl);

    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'video-series__playlist-item d-flex';
    btn.id = `video-related-${itemIndex}`;
    btn.setAttribute('aria-controls', itemId);

    const thumbDiv = document.createElement('div');
    thumbDiv.className = 'video-series__playlist-thumbnail';

    if (thumb && thumb.src) {
      const img = document.createElement('img');
      img.className = 'image';
      img.src = thumb.src;
      img.alt = thumb.alt || `Thumbnail for video ${itemIndex}.`;
      if (thumb.title) img.title = thumb.title;
      if (thumb.width) img.width = Number(thumb.width) || undefined;
      if (thumb.height) img.height = Number(thumb.height) || undefined;
      img.loading = 'lazy';
      thumbDiv.appendChild(img);
    }

    const descDiv = document.createElement('div');
    descDiv.className = 'video-series__playlist-description';

    const h5 = document.createElement('h5');
    h5.className = 'video-series__item-heading';
    h5.textContent = `Video ${itemIndex}:`;

    const h4 = document.createElement('h4');
    h4.className = 'video-series__item-title';
    h4.textContent = label || `Video ${itemIndex}`;

    descDiv.appendChild(h5);
    descDiv.appendChild(h4);

    btn.appendChild(thumbDiv);
    btn.appendChild(descDiv);

    btn.addEventListener('click', () => {
      loadVideoForItem(itemEl);
      itemEl.scrollIntoView({ behavior: 'smooth', block: 'start' });
      btn.focus({ preventScroll: true });
    });

    playlistWrap.appendChild(btn);
  });

  block.textContent = '';
  block.appendChild(videosWrap);
  block.appendChild(playlistWrap);
}

export default async function decorate(block) {
  const items = parseItemsFromBlock(block);

  if (!items.length) {
    block.textContent = '';
    return;
  }

  if (isUniversalEditorMode(block)) {
    block.classList.add('video-ue');
    items.forEach(({ row }) => row.classList.add('video-item-ue'));
    return;
  }

  decoratePublished(block, items);
}
